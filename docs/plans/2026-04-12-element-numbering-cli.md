...
### What Already Exists ✅
- **Model**: Element struct has `SceneNo string` field
...
Verified:
- ✅ Element struct has SceneNo field
- ✅ Schema allows sceneNo on elements
- ✅ Validation functions exist
- ✅ FDX Paragraph has Number field
- ✅ Fountain Element has SceneNo field

### Phase 2: FDX Format Implementation (In Progress)
...
**New Tests**: 13+ new test cases from Phases 2-3

**Schema Validation**: Verify sceneNo on Action/Character/Dialogue elements validates

**Manual Integration**: Round-trip testing with FDX and Fountain files

## Files to Modify

### FDX Format
- `internal/formats/fdx/bridge/to_screenjson.go`
- `internal/formats/fdx/bridge/from_screenjson.go`
- `internal/formats/fdx/bridge/to_screenjson_test.go` (+6 tests)

### Fountain Format
- `internal/formats/fountain/codec/decode.go`
- `internal/formats/fountain/bridge/to_screenjson.go`
- `internal/formats/fountain/bridge/from_screnejson.go`
- `internal/formats/fountain/bridge/to_screenjson_test.go` (+7 tests)

**Total**: 5 files modified, ~13 tests added

## Pattern Reference

**Validation Pattern** (reuse): `^[A-Za-z0-9][A-Za-z0-9.\/-]*$`

**Valid Examples**: "42", "1A", "110A", "I-1-A", "A-B-C", "1.", "110A/111B"

**Helper Functions** (reuse as-is):
- `model.NormalizeSceneNumber()` - Strips # delimiters
- `model.ValidateSceneNumber()` - Validates format

## Success Criteria

- ✅ All existing tests pass
- ✅ 13+ new tests passing
- ✅ FDX import extracts Paragraph.Number for Action/Character/Dialogue
- ✅ Fountain import parses #NUMBER# syntax
- ✅ Round-trip conversions preserve numbers
